...
### Node JS
  ...
  * const 블록 스코프를 갖는다. 상수를 선언할 때 사용하며, 선언과 동시에 할당 되어야 한다. 다른 값으로 재할당 시에 오류가 발생한다.(재할당 불가)
  ...
  #### Hoisting(호이스팅)
  변수를 선언할 때 어떤 위치에 있더라고 함수 시작 위치로 끌어올려 상용한다.
  
  #### collback 함수
  나중에 호출되는 함수, 특별한 선언이나 문법적인 특징을 가지고 있지 않다. <Br>
  어떤 이벤트가 발생했거나 특정 시점에 도달했을 때 시스템에서 호출하는 함수를 말한다.
  
  #### Promise
  new Promise()로 가져오며 resolve, reject를 갖는다.<br>
  서버에서 받아온 데이터를 화면에 표시하기 위해 사용한다.<br>
  pending 비동기 처리가 아직 수행되지 않은 상태 즉 대기상태<br>
  fulfilled 비동기 처리가 수행되여 결과값을 이미 리턴해준 상태<br>
  rejected 비동기 처리가 수행되었지만 실패하여 오류가 발생한 상태<br>
  settled 비동기처리가 수행 된 상태, 실패 또는 성공
