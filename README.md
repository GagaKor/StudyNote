...
### Node JS
  ...
  * const 블록 스코프를 갖는다. 상수를 선언할 때 사용하며, 선언과 동시에 할당 되어야 한다. 다른 값으로 재할당 시에 오류가 발생한다.(재할당 불가)
  ...
  #### Promise
  new Promise()로 가져오며 resolve, reject를 갖는다.
  * 서버에서 받아온 데이터를 화면에 표시하기 위해 사용한다.
  * pending 비동기 처리가 아직 수행되지 않은 상태 즉 대기상태
  * fulfilled 비동기 처리가 수행되여 결과값을 이미 리턴해준 상태
  * rejected 비동기 처리가 수행되었지만 실패하여 오류가 발생한 상태
